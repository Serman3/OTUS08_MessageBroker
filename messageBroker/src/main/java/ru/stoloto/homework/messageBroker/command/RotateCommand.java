package ru.stoloto.homework.messageBroker.command;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.context.annotation.Scope;
import org.springframework.stereotype.Component;
import ru.stoloto.homework.messageBroker.annotation.Id;
import ru.stoloto.homework.messageBroker.model.GameItem;

import java.util.Map;

@Id("ROTATE")
@Component
@Scope("prototype")
public class RotateCommand implements Command {

    private static final Logger LOGGER = LoggerFactory.getLogger(RotateCommand.class);

    private final GameItem gameObject;
    private final Map<String, Object> args;

    public RotateCommand(GameItem gameObject, Map<String, Object> args) {
        this.gameObject = gameObject;
        this.args = args;
    }

    @Override
    public void execute() {
        Integer angle = Integer.parseInt((String) args.get("angle"));
        gameObject.setProperty("angle", angle);
        LOGGER.info("Rotating object {} with angle {}", gameObject.getId(), angle);
    }
}
